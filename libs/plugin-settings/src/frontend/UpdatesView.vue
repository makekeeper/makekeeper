...
import {
  buildUpdatesNav,
  DEFAULT_UPDATE_SECTION,
  isUpdateSection,
  type UpdateSectionKey,
} from './updates-sections';
...
import CountingSection from './CountingSection.vue';
...
//
// The section list itself moved out to `updates-sections.ts` when About joined
// it (#342): About renders from its own route — it is the one section that is
// not admin territory — and both pages have to show the same picker or the
...
const sectionQuery = useRouteQuery('section', {
  default: DEFAULT_UPDATE_SECTION,
});
const section = computed<UpdateSectionKey>(() =>
  isUpdateSection(sectionQuery.value)
    ? sectionQuery.value
    : DEFAULT_UPDATE_SECTION,
);

// Admin territory throughout (`meta.adminOnly` on the route), so the picker
// always carries the full list here.
const items = computed(() =>
  buildUpdatesNav({
    query: route.query,
    t,
    updateAvailable: store.updateAvailable,
    includeAdminSections: true,
  }),
);
    ...
    <div class="grid gap-6 lg:grid-cols-[16rem_minmax(0,1fr)]">
      ...
      <div class="min-w-0">
        ...
        <KeepAlive>
          ...
          <InstallMethodSection v-else-if="section === 'install'" />
          <CountingSection v-else />
        </KeepAlive>
      </div>
    </div>
  </div>
