# MakeKeeper — the web image

**You have the part. Somewhere.** MakeKeeper is a self-hosted workshop log: what you are
building, what you have on the shelf, where exactly it is, and what is still on its way — in
one place, on your own server.

This image is the **web front** — nginx serving the single-page interface and reverse-proxying
`/api` to the backend. On its own it does nothing: a working instance is this image plus
**[`makekeeper/app`](https://hub.docker.com/r/makekeeper/app)** (the REST API and agent runtime)
and a PostgreSQL 16. The install below brings up all three.
...
`ghcr.io/makekeeper/web`, which stays the primary registry.
