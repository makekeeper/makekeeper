...
import { defaultSchedule } from '../state.ts';
import type { State } from '../state.ts';

// A key that takes a parameter must be GIVEN one.
//
// The trap this catches, in this plugin's terms: a rates table with a {base}
// nobody passed, or a staleness line missing its reason.
//
...
const state = (over: Partial<State> = {}): State => ({
  ...
  history: {},
  currencyNames: { USD: 'US Dollar', RUB: 'Russian Ruble' },
  ...over,
});

// Every shape a reader can land on, the unhappy ones included — those are the
// ones nobody looks at twice.
const SCREENS = (): UiScreen[] => [
  homeScreen(state()),
  homeScreen(state({ lastError: 'HTTP 503' })),
  homeScreen(state({ latest: null })),
  homeScreen(state({ schedule: { ...defaultSchedule(), autoRefresh: false } })),
  settingsScreen(state()),
  settingsScreen(state(), { autoRefresh: false }),
  settingsScreen(state(), { autoRefresh: true }),
  settingsScreen(state({ lastError: 'HTTP 503' })),
];
