...
import { issueCode, loadState, redeemCode, forgetScope, setBot } from '../state.ts';

// A key that takes a parameter must be GIVEN one.
//
// Telegram's own near-miss: a failed test message once passed an empty
// {detail}, so "did not accept the message ()" was all a person got.
//
...
process.env['MK_STATE_DIR'] = process.env['MK_STATE_DIR'] ?? '/tmp/mk-telegram-ph';

await loadState();
await forgetScope('scope-a');
await setBot('token', 'workshopbot');
await redeemCode(issueCode('scope-a', 'anna', 'en'), 111);

const SCREENS = (): UiScreen[] => [
  homeScreen('scope-a', 'anna', 'workshopbot'),
  homeScreen('scope-a', 'boris', 'workshopbot'),
  homeScreen('scope-a', 'boris', ''),
  homeScreen('scope-a', undefined, 'workshopbot'),
  settingsScreen(null, ''),
  settingsScreen({ ok: true, bot: 'workshopbot' }, 'https://plugin.example'),
  settingsScreen({ ok: false, detail: 'HTTP 401' }, ''),
  settingsScreen({ ok: false }, ''),
];
