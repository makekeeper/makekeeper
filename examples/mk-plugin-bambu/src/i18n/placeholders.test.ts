...
import { emptyStatus } from '../printer.ts';
import type { State } from '../state.ts';

// A key that takes a parameter must be GIVEN one.
//
// `offline` reads "Not connected to the printer ({detail})" and every caller
// passed no params, so the screen said "Not connected to the printer ()" —
// for a printer that was switched off, for a bad token, and for a plugin
// nobody had configured yet. It rendered, it validated, it said nothing.
//
...
const state = (over: Partial<State> = {}): State => ({
  ...
  jobStartedAt: null,
  log: [],
  connection: { ok: false, at: '2026-07-29T00:00:00.000Z' },
  haEntities: [],
  haCheck: null,
  ...over,
});
...
// Every shape the user can land on, including the ones that only appear when
// something is wrong — those are the ones nobody looks at twice.
const SCREENS = (): UiScreen[] => [
  homeScreen(state()),
  homeScreen(configured()),
  homeScreen({
    ...configured(),
    connection: { ok: false, detail: 'HTTP 401', at: '2026-07-29T00:00:00.000Z' },
  }),
  homeScreen({
    ...configured(),
    connection: { ok: true, at: '2026-07-29T00:00:00.000Z' },
  }),
  widgetScreen(state()),
  widgetScreen({ ...configured(), connection: { ok: true, at: 'x' } }),
  settingsScreen(state().config),
  settingsScreen(configured().config, { source: 'ha' }),
  settingsScreen(configured().config, { source: 'lan' }),
  settingsScreen(
    configured().config,
    { source: 'ha' },
    {
      entities: ['sensor.p1s_print_status', 'sensor.p1s_print_progress'],
      check: { ok: true, at: 'x' },
    },
  ),
  settingsScreen(
    configured().config,
    { source: 'ha' },
    { entities: [], check: { ok: false, detail: 'HTTP 401', at: 'x' } },
  ),
  settingsScreen(configured().config, { source: 'ha' }, { entities: [], check: { ok: false, at: 'x' } }),
];
