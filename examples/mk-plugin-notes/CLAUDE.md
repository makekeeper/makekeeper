...
- [ ] Logic that GUESSES (name matching, discovery, parsing) has a
      `node --test` spec next to it: a wrong guess does not crash, it produces
      a screen that looks configured and data that never arrives.
